...
The E2E path is local-only and destructive against local test state. It seeds a demo WorldVersion, opens disposable Adventures, verifies Player Card and Room Info surfaces, verifies Story inserts and hidden Guide turns, exercises Adventure-scoped turn/debug/reset behavior, and expects source WorldVersion identity to remain visible in debug evidence. Confirm configured ports are free before assuming failures are app regressions. In particular, `npm run dev:debug` also uses the local Convex port `3210`; stop that server before running E2E, then restart it afterward if you need the playtest app left running.
