...
The next slice should make the player a first-class Adventure-owned character card without adding RPG systems prematurely.
...
- Product: Adds a persistent protagonist identity surface and makes each new Adventure feel more like a character-led story.
- Code: Requires updates to Adventure creation, baseline copy, snapshot/context read models, prompt construction, and play UI layout.
- Tests: Requires focused tests for Adventure creation/player facts, prompt context, and deterministic E2E or browser coverage for the name prompt and collapsible Player Card.
...
- Required: yes
- Category: Added
- Public summary: Add player naming at Adventure creation and a persistent Player Card for character context.

## Open Questions

- None blocking. During implementation, if inline editing in the Player Card proves too busy, the fallback is a compact edit mode within the same left rail rather than moving the feature into debug.
