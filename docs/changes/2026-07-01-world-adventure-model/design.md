...
- Keep the default Stormbound Chapel playtest simple: seed a WorldVersion and create or resume a default local Adventure.
...
- A polished Adventure picker or public World Builder.
...
- Client surfaces: Current browser can still open the default Adventure; future clients can target Adventure APIs.
...
Implementation should keep the player-facing MVP simple. On local seed/repair, the app should create the Stormbound Chapel World, create an initial immutable WorldVersion, and create or resume a default Adventure from that version. The current play UI can continue to open the default Adventure automatically; a polished Adventure picker is deferred.
...
- Keep the first player-facing path as one automatically selected default Adventure.
