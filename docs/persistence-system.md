...
This keeps playtesting focused on the initial seed, transcript behavior, prompt shape, and Game Master loop. The MVP has a lightweight local World container screen at `/` that lists Adventures inside Stormbound Chapel for continue/create. Each Adventure opens at `/adventures/<id>`. Polished World management, source patching, branching, and long-lived save files are deferred until the core story loop is worth preserving.
